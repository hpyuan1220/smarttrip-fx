...
- **撿便宜・最後一分鐘**：精選特價 / 錯誤票 / 剩房情報站 —— 機票(Secret Flying、Going、FunTime 廉航比價、吃貨瑪莉) 與飯店(HotelTonight、lastminute.com、Agoda 限時優惠、LateRooms)，臨時起意也能撿到好價。
...
- **匯率燈號(即時)**：預設以**免金鑰即時匯率**讀取「消費幣別 / 本國幣別」近 30 天資料,計算 MA30 並與今日比較產生燈號;來源失敗時才退回(以美元交叉匯率的)模擬資料,並於面板標示「模擬資料 / 即時資料」。
...
| `EXCHANGE_API_KEY` | (選填)exchangerate.host 備援金鑰。預設已使用免金鑰即時匯率;兩者皆失敗才退回模擬資料。 |

> 不需任何金鑰即可完整體驗:匯率預設使用**免金鑰即時來源**(fawazahmed0 currency-api,支援 TWD 等多數幣別、含每日歷史);行程在未設定 OpenAI 金鑰時使用內建示範範本。
